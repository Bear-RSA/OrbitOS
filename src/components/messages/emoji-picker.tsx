...
/*  Emoji grid                                                         */
/*                                                                     */
/*  Content only — the popover, the tabs and the click-away belong to  */
/*  `media-picker`, which shows this beside the GIF and sticker grids. */
/*                                                                     */
...
/*  starts at 😀 makes them hunt for 👍 every time. Per-device, in     */
/*  localStorage, because it is a convenience and not a fact about the */
/*  account.                                                           */
/* ------------------------------------------------------------------ */
...
export function EmojiGrid({ onSelect }: { onSelect: (emoji: string) => void }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="custom-scrollbar flex shrink-0 gap-0.5 overflow-x-auto border-t border-line/[0.04] p-1.5">
        {EMOJI_GROUPS.map((g) => (
          <button
            key={g.name}
            type="button"
