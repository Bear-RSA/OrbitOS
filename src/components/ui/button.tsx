...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-[13px] font-medium transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/40 disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98]",
  {
    variants: {
      variant: {
        default: 
          "bg-primary text-primary-foreground shadow-[0_2px_12px_rgb(var(--ink-strong)_/_0.06),0_8px_24px_rgb(var(--scrim)_/_0.3)] hover:brightness(1.04) active:scale-[0.97]",
        destructive: 
        ...
        secondary: 
          "bg-gradient-to-b from-surface-active to-surface-control text-ink shadow-[inset_0_1px_0_rgb(var(--ink-strong)_/_0.06),0_2px_4px_rgb(var(--scrim)_/_0.4)] hover:from-surface-active hover:to-surface-hover active:scale-[0.97]",
        ghost: 
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, isLoading = false, children, disabled, ...props }, ref) => {
    ...
    const loaderColor = variant === 'default' ? themeColor.onInk : 
                        variant === 'destructive' ? themeColor.red : 
                        themeColor.ink;
