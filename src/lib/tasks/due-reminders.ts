...
/*                                                                     */
/*  The reminder goes out with the rest of the morning cron rather     */
/*  than at a fixed 24h mark — a due date is a calendar day, not an    */
/*  instant, so there is no hour to be exact about. Move the schedule  */
/*  in vercel.json and the lead time moves with it; the selection      */
/*  stays "due tomorrow".                                              */
/*                                                                     */
...
export async function runDueTaskReminders(options?: {
  ...
  const targetDateKey = sastNextDayKey(now);
  ...
  for (let i = 0; i < toMark.length; i += 400) {
    ...
    for (const taskId of toMark.slice(i, i + 400)) {
      ...
      // would tell other inactivity checks that a stalled task had just
      // been worked on.
