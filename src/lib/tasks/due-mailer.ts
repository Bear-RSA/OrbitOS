...
import { resolvePreferences, type UserPreferences } from "@/types/preferences";

/* ------------------------------------------------------------------ */
/*  Due-date mailer — shared machinery                                 */
/*                                                                     */
/*  Two scheduled mails ask the same question a day apart: who needs   */
/*  telling about work due on a given calendar day? The due-tomorrow   */
/*  reminder asks it at 09:00 about tomorrow; the due-today digest     */
/*  asks it at 06:00 about today. Everything between the question and  */
/*  the send is identical — the same owner fallback for unclaimed      */
/*  work, the same opt-out check, the same tier and hard ceilings, the */
/*  same per-task idempotency marker, the same wave pacing.            */
/*                                                                     */
/*  So it lives here once, and the two callers supply only what        */
/*  genuinely differs: which day, which marker field, which preference */
/*  gates it, and which template sends it. A second copy of this file  */
/*  is how the two mails would end up disagreeing about who is allowed */
/*  to receive one.                                                    */
/*                                                                     */
/*  Idempotency is per task, per day: the marker holds the due-date    */
/*  key already mailed about, so a retry inside the same day is a      */
/*  no-op while moving a task to another day earns a fresh send.       */
/* ------------------------------------------------------------------ */

/**
 * Hard ceilings. ALWAYS on, independent of BILLING_GUARDRAILS_ENABLED,
 * because every send is a Resend invoice charged to us. A tier may narrow
...
/** A mail listing forty tasks is a wall, not a prompt. */
const MAX_TASKS_LISTED = 25;

export interface DueMailRunResult {
  /** The due-date key mailed about. */
...
export interface DueMailPayload {
...
export interface DueMailerConfig {
  /** The calendar day this run is about, given the moment it ran. */
  targetKeyFor: (now: Date) => string;
  /** Task field holding the day already mailed about. */
  markerField: string;
  /** The Settings -> Notifications toggle that gates this mail. */
  preferenceKey: keyof Pick<UserPreferences, "taskReminders" | "dueTodayDigest">;
  /** Reason recorded against somebody who has opted out. */
  optOutReason: string;
  send: (payload: DueMailPayload) => Promise<SendResult>;
}
...
/**
 * Orders a mail for reading: unassigned first, since those need somebody to
 * ACT rather than merely to know, then alphabetically so the same list
 * arrives in the same order every time.
 */
 ...
 * recipient that rotates between runs makes the mail feel random, and
 ...
 * send on it.
 */
export async function runDueMailer(
  config: DueMailerConfig,
  options?: { now?: Date; dryRun?: boolean }
): Promise<DueMailRunResult> {
  const now = options?.now ?? new Date();
  const dryRun = options?.dryRun ?? false;
  const targetDateKey = config.targetKeyFor(now);
  ...
  const candidates = docs.filter((doc) => {
    ...
    return task[config.markerField] !== targetDateKey;
  });
  ...
  for (const doc of candidates) {
    ...
    for (const uid of recipients) {
      ...
      if (!resolvePreferences(user.preferences)[config.preferenceKey]) {
        if (!droppedUids.has(uid)) {
          droppedUids.add(uid);
          skipped.push(`${user.email} (${config.optOutReason})`);
        }
        continue;
      }
  ...
  // recipient failing must not silence the mail for the other.
